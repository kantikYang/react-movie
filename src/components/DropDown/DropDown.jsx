...
const DropList = (items, dropRef) => {
  console.log('ffddsfsdff');
  if (!dropRef.current) return null;
  console.log('ffffff');
  const pos = dropRef.current.getBoundingClientRect();
...
export const DropDown = ({ title, placeHolder, isDrop, items }) => {
  ...
  return (
    <div>
      <Input title={title} placeHolder={placeHolder} isDrop={true}>
        <button onClick={() => setDrop(!drop)} className={style.buttonInput}>
          {arrowBtn}
        </button>
      </Input>
      {drop && (
      ...
      {/* {drop &&
        createPortal(
          DropList(items, dropRef),
          document.body //.querySelector < HTMLDivElement > '#modal-root'
        )} */}
    </div>
  );
};
