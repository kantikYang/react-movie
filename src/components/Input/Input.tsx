...
export const Input = ({
  ...
  return (
    <div className={style.itemInput}>
      <p className={style.titleInput}>{title}</p>
      <div className={style.wrapperInut}>
        <input
          className={style.placeInput}
          type="text"
          readOnly={isDrop ? true : false}
          placeholder={placeHolder}
        />
        {children}
        {/* {isDrop && <button className={style.buttonInput}>{arrowBtn}</button>} */}
      </div>
    </div>
  );
};
